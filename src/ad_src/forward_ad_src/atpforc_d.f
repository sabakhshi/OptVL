...
C...Sum velocity contributions from wake vortices
        DO jv=1,nstrip
          temp = rt2(2, jv) - rt1(2, jv)
          temp0 = rt2(3, jv) - rt1(3, jv)
          arg1_diff = 2*temp*(rt2_diff(2, jv)-rt1_diff(2, jv)) + 2*temp0
     +      *(rt2_diff(3, jv)-rt1_diff(3, jv))
...
C
          ...
          dz2 = zcntr - rt2(3, jv)
          temp0 = dy1*dy1 + dz1*dz1
          arg1_diff = 2*temp0*(2*dy1*dy1_diff+2*dz1*dz1_diff) + 4*rcore
     +      **3*rcore_diff
          arg1 = temp0*temp0 + rcore**4
          temp0 = SQRT(arg1)
          IF (arg1 .EQ. 0.D0) THEN
            rsq1_diff = 0.D0
          ELSE
            rsq1_diff = arg1_diff/(2.0*temp0)
          END IF
          rsq1 = temp0
          temp0 = dy2*dy2 + dz2*dz2
          arg1_diff = 2*temp0*(2*dy2*dy2_diff+2*dz2*dz2_diff) + 4*rcore
     +      **3*rcore_diff
          arg1 = temp0*temp0 + rcore**4
          temp0 = SQRT(arg1)
          IF (arg1 .EQ. 0.D0) THEN
...
CCC         DZ2 = ZCNTR - (ZOFF-RT2(3,JV)+ALFA*RT2(1,JV))
            ...
            temp0 = dz1/rsq1 - dz2/rsq2
            vy_diff = vy_diff - hpi*izsym*(temp0*gams_diff(jv)+gams(jv)*
     +        ((dz1_diff-dz1*rsq1_diff/rsq1)/rsq1-(dz2_diff-dz2*
     +        rsq2_diff/rsq2)/rsq2))
            vy = vy - hpi*izsym*(gams(jv)*temp0)
            temp0 = dy2/rsq2 - dy1/rsq1
            vz_diff = vz_diff - hpi*izsym*(temp0*gams_diff(jv)+gams(jv)*
     +        ((dy2_diff-dy2*rsq2_diff/rsq2)/rsq2-(dy1_diff-dy1*
     +        rsq1_diff/rsq1)/rsq1))
            vz = vz - hpi*izsym*(gams(jv)*temp0)
            DO n=1,numax
              vy_u(n) = vy_u(n) - hpi*gams_u(jv, n)*(dz1/rsq1-dz2/rsq2)*
     +          izsym
...
C
          IF (iysym .NE. 0) THEN
            ...
            rsq2 = dy2*dy2 + dz2*dz2
            temp0 = dz1/rsq1 - dz2/rsq2
            vy_diff = vy_diff - hpi*iysym*(temp0*gams_diff(jv)+gams(jv)*
     +        ((dz1_diff-dz1*rsq1_diff/rsq1)/rsq1-(dz2_diff-dz2*
     +        rsq2_diff/rsq2)/rsq2))
            vy = vy - hpi*iysym*(gams(jv)*temp0)
            temp0 = dy2/rsq2 - dy1/rsq1
            vz_diff = vz_diff - hpi*iysym*(temp0*gams_diff(jv)+gams(jv)*
     +        ((dy2_diff-dy2*rsq2_diff/rsq2)/rsq2-(dy1_diff-dy1*
     +        rsq1_diff/rsq1)/rsq1))
            vz = vz - hpi*iysym*(gams(jv)*temp0)
            DO n=1,numax
              vy_u(n) = vy_u(n) - hpi*gams_u(jv, n)*(dz1/rsq1-dz2/rsq2)*
     +          iysym
              vz_u(n) = vz_u(n) - hpi*gams_u(jv, n)*(-(dy1/rsq1)+dy2/
...
CCC           DZ2 = ZCNTR - (ZOFF-RT2(3,JV)+ALFA*RT2(1,JV))
              ...
              vy_diff = vy_diff + temp0*(temp*gams_diff(jv)+gams(jv)*((
     +          dz1_diff-dz1*rsq1_diff/rsq1)/rsq1-(dz2_diff-dz2*
     +          rsq2_diff/rsq2)/rsq2))
              vy = vy + temp0*(gams(jv)*temp)
              temp0 = hpi*iysym*izsym
...
C
C---- aspect ratio
      temp1 = bref*bref/sref
      ar_diff = (2*bref*bref_diff-temp1*sref_diff)/sref
      ar = temp1
C
C---- span efficiency
      IF (cdff .EQ. 0.0) THEN
...
C
        temp1 = pi*ar*cdff
        temp2 = (clff*clff+cyff*cyff)/temp1
        spanef_diff = (2*clff*clff_diff+2*cyff*cyff_diff-temp2*(cdff*pi*
